...
import { Home, ClipboardCheck, CalendarCheck, Settings, LogOut, Menu, X } from 'lucide-react';
    ...
    <>
      ...
      <nav className="flex-1 px-4 py-4 overflow-y-auto space-y-1 min-h-0">
        {navItems.map((item, idx) => {
          ...
          return (
            <Link key={idx} to={item.path} onClick={() => setIsMobileOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${isActive ? 'bg-amber-50 text-amber-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
