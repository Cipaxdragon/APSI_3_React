...
import { Home, ClipboardList, Calendar, Users, Settings, LogOut, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      <button 
        ...
        className="lg:hidden fixed bottom-6 right-6 z-[60] bg-blue-600 text-white p-3.5 rounded-full shadow-2xl hover:bg-blue-700 transition-colors focus:outline-none"
      >
      ...
      <nav className="flex-1 px-4 py-6 overflow-y-auto space-y-1">
        {navItems.map((item, idx) => {
          ...
          return (
            <Link key={idx} to={item.path} onClick={() => setIsMobileOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${isActive ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
