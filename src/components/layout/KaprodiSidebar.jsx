...
import { Home, ClipboardCheck, CalendarCheck, Settings, LogOut } from 'lucide-react';
    ...
    <aside className="w-64 bg-white border-r border-slate-100 flex flex-col fixed inset-y-0 left-0 z-50 h-screen">
      ...
      <nav className="flex-1 px-4 py-4 overflow-y-auto space-y-1 min-h-0">
        {navItems.map((item, idx) => {
          ...
          return (
            <Link key={idx} to={item.path} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${isActive ? 'bg-amber-50 text-amber-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
